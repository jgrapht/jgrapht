...
public class ImmutableBigGraphAdapterEndpointPair
    extends
    AbstractGraph<Long, EndpointPair<Long>>
    implements
    FlyweightPrototype<ImmutableBigGraphAdapterEndpointPair>
{
     ...
     */
    public static ImmutableBigGraphAdapterEndpointPair undirected(
     ...
     */
    public static ImmutableBigGraphAdapterEndpointPair directed(
    ...
    protected ImmutableBigGraphAdapterEndpointPair(
        final ImmutableGraph immutableGraph, final ImmutableGraph immutableTranspose)
    {
    ...
    public boolean addEdge(
        final Long sourceVertex, final Long targetVertex, final EndpointPair<Long> e)
    {
        throw new UnsupportedOperationException();
    }
    ...
    public Set<EndpointPair<Long>> edgesOf(final Long source)
    {
        ...
        if (directed) {
            ...
            final LazyLongIterator predecessors = immutableTranspose.successors(source);
            for (long target; (target = predecessors.nextLong()) != -1;)
                set.add(EndpointPair.unordered(target, source));
        }
        return set;
    }
    ...
    {
        if (directed)
            return new ImmutableBigGraphAdapterEndpointPair(
    ...
    private final GraphIterables<Long, EndpointPair<Long>> ITERABLES = new GraphIterables<>()
    {
        ...
        private Iterable<EndpointPair<Long>> incomingEdgesOf(final long x, final boolean skipLoops)
        {
            return () -> new Iterator<>()
            {
        ...
        public Iterable<EndpointPair<Long>> outgoingEdgesOf(final Long vertex)
        {
            return () -> new Iterator<>()
            {
                ...
                public EndpointPair<Long> next()
                {
                    final EndpointPair<Long> edge = directed ? EndpointPair.ordered(vertex, y)
