...
public class ImmutableBigGraphAdapterLongArray
    extends
    AbstractGraph<Long, long[]>
    implements
    FlyweightPrototype<ImmutableBigGraphAdapterLongArray>
{
     ...
     */
    public static ImmutableBigGraphAdapterLongArray directed(
    ...
    protected ImmutableBigGraphAdapterLongArray(
        final ImmutableGraph immutableGraph, final ImmutableGraph immutableTranspose)
    {
    ...
    public Set<long[]> getAllEdges(final Long sourceVertex, final Long targetVertex)
    {
        ...
        if (x < 0 || x >= n || y < 0 || y >= n)
            return null;
        return containsEdgeFast(x, y) ? Collections.singleton(new long[] { x, y })
    ...
    public Set<long[]> incomingEdgesOf(final Long vertex)
    {
        ...
        for (long target; (target = predecessors.nextLong()) != -1;)
            set.add(new long[] { target, vertex });
        return set;
    }
    ...
    public Set<long[]> outgoingEdgesOf(final Long vertex)
    {
        ...
        if (directed)
            for (long target; (target = successors.nextLong()) != -1;)
                set.add(new long[] { vertex, target });
        else
            for (long target; (target = successors.nextLong()) != -1;)
                set.add(new long[] { vertex, target });
        return set;
    ...
    public Iterable<long[]> outgoingEdgesIterableOf(final Long vertex)
    {
        return () -> new Iterator<>()
        {
            final LongIterator successors =
                LazyLongIterators.eager(immutableGraph.successors(vertex));
    ...
    {
        if (directed)
            return new ImmutableBigGraphAdapterLongArray(
