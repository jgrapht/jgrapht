...
/**
 * Compressed Sparse Row Matrix with byte values.
 * 
 * @author Dimitrios Michail
 */
public class CSRByteMatrix
    implements
    Serializable
{
     ...
     */
    public CSRByteMatrix(int rows, int columns, List<Triple<Integer, Integer, Byte>> entries)
    {
        if (rows < 1) {
        ...
        Iterator<Triple<Integer, Integer, Byte>> it =
            entries.stream().sorted(new TripleComparator()).iterator();
        Triple<Integer, Integer, Byte> prev = null;
        while (it.hasNext()) {
            Triple<Integer, Integer, Byte> e = it.next();
     ...
     */
    public Iterator<Pair<Integer, Byte>> nonZerosIterator(int row)
    {
        assert row >= 0 && row < rowOffsets.length;
     ...
     */
    public Set<Pair<Integer, Byte>> rowSet(int row)
    {
        assert row >= 0 && row < rowOffsets.length;
        ...
        AbstractSet<Pair<Integer, Byte>>
    {
        ...
        @Override
        public Iterator<Pair<Integer, Byte>> iterator()
        {
            return new ColumnIterator(row);
        }
        ...
        Iterator<Pair<Integer, Byte>>
    {
        ...
        @Override
        public Pair<Integer, Byte> next()
        {
            if (!hasNext()) {
                throw new NoSuchElementException();
            }
            Pair<Integer, Byte> value = Pair.of(columnIndices[curPos], columnValues[curPos]);
            curPos++;
            return value;
        }
    ...
    private class TripleComparator
        implements
        Comparator<Triple<Integer, Integer, Byte>>
    {

        @Override
        public int compare(Triple<Integer, Integer, Byte> o1, Triple<Integer, Integer, Byte> o2)
        {
