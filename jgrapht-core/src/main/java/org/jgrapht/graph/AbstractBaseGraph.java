 ...
 * {@link GraphSpecificsStrategy} that is provided from the constructor. All
 * implemented strategies guarantee deterministic vertex and edge set ordering
 * (via {@link LinkedHashMap} and {@link LinkedHashSet}). The defaults are
 * reasonable for most use-cases, only change if you know what you are doing.
 *
...
public abstract class AbstractBaseGraph<V, E> extends AbstractGraph<V, E>
	 ...
	 * @param edgeSupplier           the edge supplier, can be null
	 * @param type                   the graph type
	 * @param graphSpecificsStrategy strategy for constructing low-level graph
	 *                               specifics
	 *
	 * @throws IllegalArgumentException if the graph type is mixed
	 */
	...
	/**
	 * Set the edge supplier that the graph uses whenever it needs to create new
	 * edges.
	 * 
	 * <p>
	 * A graph uses the edge supplier to create new edge objects whenever a user
	 * calls method {@link Graph#addEdge(Object, Object)}. Users can also create the
	 * edge in user code and then use method
	 * {@link Graph#addEdge(Object, Object, Object)} to add the edge.
	 * 
	 * <p>
	 * In contrast with the {@link Supplier} interface, the edge supplier has the
	 * additional requirement that a new and distinct result is returned every time
	 * it is invoked. More specifically for a new edge to be added in a graph
	 * <code>e</code> must <i>not</i> be equal to any other edge in the graph (even
	 * if the graph allows edge-multiplicity). More formally, the graph must not
	 * contain any edge <code>e2</code> such that <code>e2.equals(e)</code>.
	 * 
	...
	/**
	 * Set the vertex supplier that the graph uses whenever it needs to create new
	 * vertices.
	 * 
	 * <p>
	 * A graph uses the vertex supplier to create new vertex objects whenever a user
	 * calls method {@link Graph#addVertex()}. Users can also create the vertex in
	 * user code and then use method {@link Graph#addVertex(Object)} to add the
	 * vertex.
	 * 
	 * <p>
	 * In contrast with the {@link Supplier} interface, the vertex supplier has the
	 * additional requirement that a new and distinct result is returned every time
	 * it is invoked. More specifically for a new vertex to be added in a graph
	 * <code>v</code> must <i>not</i> be equal to any other vertex in the graph.
	 * More formally, the graph must not contain any vertex <code>v2</code> such
	 * that <code>v2.equals(v)</code>.
	 * 
	 * <p>
	 * Care must also be taken when interchanging calls to methods
	 * {@link Graph#addVertex(Object)} and {@link Graph#addVertex()}. In such a case
	 * the user must make sure never to add vertices in the graph using method
	 * {@link Graph#addVertex(Object)}, which are going to be returned in the future
	 * by the supplied vertex supplier. Such a sequence will result into an
	 * {@link IllegalArgumentException} when calling method
	 * {@link Graph#addVertex()}.
	 * 
	...
	/**
	 * Returns a shallow copy of this graph instance. Neither edges nor vertices are
	 * cloned.
	 *
