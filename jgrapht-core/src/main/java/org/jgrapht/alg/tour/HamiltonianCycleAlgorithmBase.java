    ...
    HamiltonianCycleAlgorithm<V, E>
{
     ...
     */
    protected static final void reverse(int[] arr, int from, int to)
    {
     ...
     * @param to the index of the last element (inclusive) inside the range to reverse
     */
    protected static final <V> void reverse(V[] arr, int from, int to)
    {
        for (int i = from, j = to; i < j; ++i, --j) {
            swap(arr, i, j);
        }
     ...
     */
    protected static final <V> void swap(V[] arr, int i, int j)
    {
        V tmp = arr[j];
        arr[j] = arr[i];
        arr[i] = tmp;
