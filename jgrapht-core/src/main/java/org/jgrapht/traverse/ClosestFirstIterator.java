...
public class ClosestFirstIterator<V, E> extends CrossComponentIterator<V, E, AddressableHeap.Handle<Double, ClosestFirstIterator.QueueEntry<V, E>>>
{
     ...
     * @param g the graph to be iterated.
     * @param startVertex the vertex iteration to be started.
     * @param radius limit on weighted path length, or {@link Double#POSITIVE_INFINITY} for
     *        unbounded search.
     *
     * @throws NullPointerException if {@code g} is {@code null}
     */
     ...
     * start at the specified start vertex and will be limited to the subset of the connected
     * component which includes that vertex and is reachable via paths of weighted length less than
     * or equal to the specified radius. The specified start vertex may not be {@code null}. This
     ...
     * @throws IllegalArgumentException if {@code startVertices} contains an element that is not a
     *         vertex of {@code g}
     * @throws NullPointerException if either one of {@code g} or {@code heapSupplier} is
     *         {@code null}
     */
    public ClosestFirstIterator(
     ...
     * @return weighted length of shortest path known, or {@link Double#POSITIVE_INFINITY} if no
     *         path found yet
     */
     ...
     * @return the spanning tree edge, or {@code null} if the vertex either has not been seen yet or
     *         is a start vertex.
     */
