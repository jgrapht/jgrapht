...
The classes in `org.jgrapht.perf.shortestpath.osm` build real road graphs from
two gzipped CSV files that live in this directory:

- `andorra-edges.csv.gz` &mdash; `src,dst,weight_m` per directed edge in the
  largest strongly-connected component of the Andorra road network.
- `andorra-edges.nodes.csv.gz` &mdash; `node_id,lat,lon` per vertex.

**These files are not committed to the repository.** They are derived from the
Geofabrik OpenStreetMap "free" GPKG extract, which is freely redistributable
under the Open Database License but is too large (and changes daily upstream)
to vendor here.
...
3. After the CSVs are in place, the smoke test and benches load automatically
   on the next `mvn test` / JMH run:

   ```
   mvn -pl jgrapht-core -Dtest=AndorraGraphLoaderSmokeTest test
   mvn -pl jgrapht-core -Dtest=AndorraBenchSuite#runM2M test
   ```
...
the CSVs under `src/test/resources/perf/osm/` &mdash; they can keep them
anywhere on disk and reference them from their own bench classes.
