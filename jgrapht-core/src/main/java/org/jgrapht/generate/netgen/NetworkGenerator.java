 ...
 * <ul>
 *     <li>Network has exactly the same number of nodes, network sources, network sinks and
 *         transshipment nodes;</li>
 ...
 * arcs respectively, but they aren't guaranteed to have.
 * <p>
...
public class NetworkGenerator<V, E> {
    ...
    public BipartiteMatchingProblem<V, E> generateBipartiteMatchingProblem(Graph<V, E> graph) {
        ...
        return new BipartiteMatchingProblem.BipartiteMatchingProblemImpl<>(graph, new HashSet<>(networkInfo.getSources()), new HashSet<>(networkInfo.getSinks()), e -> costMap.get(e), config.isCostWeighted());
    }
    ...
    public MaximumFlowProblem<V, E> generateMaxFlowProblem(Graph<V, E> graph) {
        ...
        return new MaximumFlowProblem.MaximumFlowProblemImpl<>(graph, new HashSet<>(networkInfo.getSources()), new HashSet<>(networkInfo.getSinks()), e -> capacityMap.get(e));
    }
