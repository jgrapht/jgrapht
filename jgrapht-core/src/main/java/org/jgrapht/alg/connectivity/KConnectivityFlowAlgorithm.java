 ...
 * <a href="https://www.cse.msu.edu/~cse835/Papers/Graph_connectivity_revised.pdf"> this document
 * </a>.
 *
 ...
 * <a href="https://www.cse.msu.edu/~cse835/Papers/Graph_connectivity_revised.pdf"> document </a> on
...
public class KConnectivityFlowAlgorithm<V, E>
{
    ...
    {
        this(
            graph, (g) -> new EdmondsKarpMFImpl<V, E>(g),
            (g) -> new EdmondsKarpMFImpl<Integer, DefaultEdge>(g));
    }

    /**
     * Constructs a k-connectivity algorithm for the given graph. Use algorithms given by the
     * suppliers for edge and vertex flow algorithm A supplier is a function which takes a graph and
     * construct the algorithm which will be used
     *
     * @param graph the input graph
     ...
     */
    public KConnectivityFlowAlgorithm(
        Graph<V, E> graph,
    ...
    public int getEdgeConnectivity()
    {
        if (edgeConnectivity == -1) {
            edgeConnectivity = graph.getType().isDirected() ? computeDirectedEdgeConnectivity()
    ...
    private int computeConnectivityFromNonNeighbors(V minDegreeVertex)
    {
        ...
        for (V vertex : graph.vertexSet()) {
            if (vertex.equals(minDegreeVertex))
                continue;

            if (!graph.containsEdge(minDegreeVertex, vertex)) {
                minConnectivity = Math
                    .min(minConnectivity, computeLocalVertexConnectivity(minDegreeVertex, vertex));
            }

            if (!graph.containsEdge(vertex, minDegreeVertex)) {
                minConnectivity = Math
                    .min(minConnectivity, computeLocalVertexConnectivity(vertex, minDegreeVertex));
            }
        }
        ...
        for (int i = 0; i < neighbors.size(); i++) {
            for (int j = i + 1; j < neighbors.size(); j++) {
                ...
                if (!graph.containsEdge(vertex1, vertex2)) {
                    minConnectivity =
                        Math.min(minConnectivity, computeLocalVertexConnectivity(vertex1, vertex2));
                }

                if (!graph.containsEdge(vertex2, vertex1)) {
                    minConnectivity =
                        Math.min(minConnectivity, computeLocalVertexConnectivity(vertex2, vertex1));
                }
            }
        }
    ...
    private int computeDirectedEdgeConnectivity()
    {
        ...
        for (int i = 0; i < vertices.size(); i++) {
            for (int j = i + 1; j < vertices.size(); j++) {
                minConnectivity = Math.min(
                    minConnectivity,
                    computeLocalEdgeConnectivity(vertices.get(i), vertices.get(j)));
            }
        }
