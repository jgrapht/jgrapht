...
- **Résultat attendu** :
```
Ratio théorique = (2n/n) × log(2n)/log(n) = 2 × (log n + 1)/log n
```
- **Tolérance** : Facteur 3× (due aux constantes cachées)

**Formule de vérification** :
```
Si t(n) = c × n × log(n), alors
t(2n)/t(n) = 2 × (log(2n)/log(n)) ≈ 2 × 1.3 = 2.6
```
...
### 8. Tests de Qualité (ModularityTests)

#### ✅ Test 8.1 : Modularité non-négative
- **Entrée** : Graphe modulaire
- **Résultat attendu** : Q ≥ 0 (ou Q ≥ -0.1 tolérance)
- **Justification** : Graphes avec structure ont Q > 0

#### ✅ Test 8.2 : Graphe complet a Q ≈ 0
- **Entrée** : K₂₀
- **Résultat attendu** : |Q| < 0.1
- **Justification** : Pas de structure modulaire

#### ✅ Test 8.3 : Cliques déconnectées ont Q élevé
- **Entrée** : 3 cliques K₅ sans arêtes entre elles
- **Résultat attendu** : Q > 0.5
- **Justification** : Structure parfaitement modulaire

**Formule de modularité** :
```
Q = Σᵢ [ (eᵢᵢ/2m) - (dᵢ/2m)² ]

où :
- eᵢᵢ = nombre d'arêtes internes à la communauté i
- dᵢ = somme des degrés dans la communauté i
...
2. **Qualité** :
   - Modularité maximisée localement
   - Q ≥ Q_random (meilleur que partition aléatoire)
...
Medium graph (500 vertices): 215 ms    ← Ratio: 5.1×
Large graph (1000 vertices): 520 ms    ← Ratio: 2.4× (depuis 500v)

Scaling verification:
n=100: 42 ms
n=200: 95 ms     ← Ratio: 2.26× (attendu: ~2.6×) ✓
n=400: 223 ms    ← Ratio: 2.35× (attendu: ~2.6×) ✓
n=800: 568 ms    ← Ratio: 2.55× (attendu: ~2.6×) ✓
```
