...
- **version 1.6.0** (Under development)
    ...
    - Added exact Hamiltonian path algorithms in new `alg.hamiltonian` package: `BacktrackingHamiltonianPath` (DFS with cut-vertex, SCC and reachability pruning), `HeldKarpHamiltonianPath` (small-graph DP), and `DagHamiltonianPath` (polynomial DAG special case) (contributed by Shai Eilat)
