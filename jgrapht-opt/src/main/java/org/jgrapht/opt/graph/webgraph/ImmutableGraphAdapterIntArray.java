...
public class ImmutableGraphAdapterIntArray extends AbstractGraph<Integer, int[]> implements FlyweightPrototype<ImmutableGraphAdapterIntArray> {
	 ...
	 */
	public static ImmutableGraphAdapterIntArray directed(final ImmutableGraph immutableGraph, final ImmutableGraph immutableTranspose) {
	...
	protected ImmutableGraphAdapterIntArray(final ImmutableGraph immutableGraph, final ImmutableGraph immutableTranspose) {
	...
	public Set<int[]> getAllEdges(final Integer sourceVertex, final Integer targetVertex) {
		...
		return containsEdgeFast(x, y) ? Collections.singleton(new int[] { x, y }) : Collections.emptySet();
	}
	...
	public Set<int[]> incomingEdgesOf(final Integer vertex) {
		...
		for (int target; (target = predecessors.nextInt()) != -1;) set.add(new int[] { target, vertex });
		return set;
	}
	...
	public Set<int[]> outgoingEdgesOf(final Integer vertex) {
		...
		if (directed) for (int target; (target = successors.nextInt()) != -1;) set.add(new int[] {vertex, target});
		else for (int target; (target = successors.nextInt()) != -1;) set.add(new int[] { vertex, target });
		return set;
	}
	...
	public ImmutableGraphAdapterIntArray copy() {
		if (directed) return new ImmutableGraphAdapterIntArray(immutableGraph.copy(), immutableTranspose.copy());
